...
> **汉化范围**：包括软件界面、顶部系统菜单、任务栏右键菜单、加载动画、设置面板、新手引导及登录页（当前版本针对v2.1.4补全了99%的汉化，仅保留简体中文）
...
### 2026-06-23
- 🛠️ **修复安装后白屏崩溃问题**：修复了汉化引擎动态代码生成时，模板字面量内部正则表达式反斜杠逃逸导致的 JS 语法错误（`Invalid regular expression flags`）。确保注入后的 `preload.js` 语法 100% 正确，不再引发启动白屏。
- 🌐 **补全浏览器设置汉化**：针对浏览器子代理（Browser Subagent）设置项中由于 HTML 链接拆分造成的遗留英文段落，补全了翻译词条。
- ⚡ **完善超额额度限制汉化**：
  - 新增了当触发 5 小时额度限制或每周额度限制（Limit Hit）时的警告文案汉化。
  - 补充了多组动态时间变量正则汉化规则，并重构了翻译引擎的 Fallback 机制，支持对同一个 DOM 节点内多个混合句子的全局局部动态翻译，彻底解决限额提示中“中英混合”或“全英文”的遗留顽疾。
- 📦 **新增 Dart & Flutter 技能描述汉化**：补全了 Dart 与 Flutter 工作区技能（Happy Path）描述的中文翻译。
